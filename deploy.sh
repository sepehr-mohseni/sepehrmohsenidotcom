...
start_db_ui() {
    ...
    log_warn "⚠️  For security, DB UI is only accessible from localhost."
...
backup_db() {
    ...
    docker run --rm \
        ...
        alpine cp /data/portfolio.db /backup/portfolio_$TIMESTAMP.db 2>/dev/null || {
...
restore_db() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        TEMP_DIR=$(mktemp -d)
        gunzip -c "$BACKUP_FILE" > "$TEMP_DIR/portfolio.db"
        
        docker run --rm \
            -v $DATA_VOLUME:/data \
            -v $TEMP_DIR:/backup:ro \
            alpine cp /backup/portfolio.db /data/portfolio.db
